...
import { Card, CardHeader, Badge, Button, FormInput, FormSelect, PageHeader } from '@/components/ui';
...
const TYPES   = ['Взнос на содержание', 'Коммунальные услуги', 'Дополнительный сбор'];
...
export default function AccrualsPage() {
  ...
  const onSubmit = (data: AccrualForm) => {
    ...
    setSuccess(true);
    reset({ period: PERIODS[0], type: TYPES[0], ratePerSqm: 57, applyTo: 'all' });
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
            ...
            <div className="bg-stone-50 border border-stone-200 rounded-lg px-4 py-3">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-stone-600 font-medium">Итого к начислению</span>
            ...
            <tbody className="divide-y divide-stone-50">
              {accruals.map((a) => (
                <tr key={a.id} className="hover:bg-stone-50">
                  <td className="px-4 py-2.5 font-medium text-stone-800">{a.period}</td>
                  <td className="px-4 py-2.5 font-mono text-stone-900">
                    {formatCurrency(a.totalAmount)}
                  </td>
                  <td className="px-4 py-2.5 text-stone-500 text-xs">{a.type}</td>
                  <td className="px-4 py-2.5 text-stone-500">{a.apartments}</td>
                  <td className="px-4 py-2.5">
                    <Badge variant={a.status === 'applied' ? 'green' : 'amber'}>
