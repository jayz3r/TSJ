...
const categoryBadgeVariant: Record<string, 'blue' | 'amber' | 'gray' | 'green' | 'red'> = {
  cleaning:   'blue',
  repair:     'amber',
  electrical: 'green',
  security:   'red',
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Card>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
            ...
            {/* Загрузка документов */}
            <div>
              <p className="text-xs font-medium text-stone-500 mb-1.5">Документы</p>
              <label className="flex flex-col items-center gap-1.5 border border-dashed border-stone-300 rounded-lg p-4 cursor-pointer hover:bg-stone-50 transition-colors">
                <span className="text-xl">📎</span>
                <span className="text-xs text-stone-400">Чек, договор, фото</span>
            ...
            <tbody className="divide-y divide-stone-50">
              {expenses.map((e) => (
                <tr key={e.id} className="hover:bg-stone-50">
                  ...
                  <td className="px-4 py-2.5 text-stone-500 text-xs max-w-32 truncate">
                    {e.description}
                  </td>
