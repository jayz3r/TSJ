      ...
      <Card className="mb-4">
        ...
        <div className="p-4 md:p-5 flex flex-col sm:flex-row gap-3 sm:items-end">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-stone-500 dark:text-stone-500">Тип</label>
            <select
              value={type}
              onChange={(e) => setType(e.target.value)}
              className="px-3 py-2 text-sm border border-stone-200 rounded-lg bg-white focus:outline-none focus:border-emerald-500 dark:bg-stone-900 dark:border-stone-700 dark:text-stone-100"
            >
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-stone-500 dark:text-stone-500">Период</label>
            <select
              value={period}
        ...
        <Card>
          ...
          <div className="md:hidden divide-y divide-stone-100">
            {rows.map((row) => (
              ...
              >
                <span
                  className={`text-sm ${row.bold ? "font-semibold text-stone-900 dark:text-stone-100" : "text-stone-500 dark:text-stone-500"}`}
                >
                  {row.label}
                </span>
                <div className="text-right">
                  ...
                  <p
                    className={`text-xs mt-0.5 ${"warn" in row && row.warn ? "text-amber-600 font-medium" : "text-stone-400 dark:text-stone-500"}`}
                  >
                    {row.note}
                  </p>
                </div>
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-stone-50">
                {rows.map((row) => (
                  <tr key={row.label} className="hover:bg-stone-50 dark:hover:bg-stone-800/60">
                    <td
                      className={`px-4 py-3 ${row.bold ? "font-semibold text-stone-900 dark:text-stone-100" : "text-stone-600 dark:text-stone-400"}`}
                    >
                      {row.label}
                    </td>
                    <td
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-xs ${"warn" in row && row.warn ? "text-amber-600 font-medium" : "text-stone-400 dark:text-stone-500"}`}
                      >
                        {row.note}
                      </span>
                    </td>
        ...
        <Card>
          ...
          <div className="p-4 md:p-5 space-y-4">
            ...
            <div className="pt-3 border-t border-stone-100 flex justify-between text-sm">
              <span className="text-stone-400 dark:text-stone-500">Итого расходов</span>
