...
export function ResidentDetailView({ backHref }: ResidentDetailViewProps) {
  ...
  if (!apt) return <p className="text-sm text-stone-400 dark:text-stone-500">Квартира не найдена.</p>;
  ...
  return (
    <div>
      <div className="mb-5">
        <Link href={backHref} className="text-sm text-stone-400 dark:text-stone-500 hover:text-stone-700 dark:hover:text-stone-300">
          ← Квартиры
        </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          ...
          {aptPayments.length === 0 ? (
            <p className="text-center py-8 text-sm text-stone-400 dark:text-stone-500">Платежей нет</p>
          ) : (
                ...
                <tbody className="divide-y divide-stone-50">
                  {aptPayments.map((p) => (
                    <tr key={p.id} className="hover:bg-stone-50 dark:hover:bg-stone-800/60">
                      <td className="px-4 py-2.5 text-stone-500 dark:text-stone-500">{formatDate(p.date)}</td>
