...
interface AppState {
  ...
  addApartment: (
    data: Omit<Apartment, "id" | "accountNumber" | "balance" | "debt">,
  ) => void; // НОВОЕ
  updateApartmentContact: (id: string, data: { phone?: string }) => void;
}
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  addApartment: (data) => {
    set((state) => {
      const nextNum = state.apartments.length + 1;
      const accountNumber = `ЛС-001${String(nextNum).padStart(2, "0")}`;
      ...
      return { apartments: [...state.apartments, newApt] };
    });
  },
    addPayment: (data) => {
  ...
  addExpense: (data) =>
    set((s) => ({
      expenses: [{ ...data, id: crypto.randomUUID() }, ...s.expenses],
    })),
  ...
  addRequest: (data) => {
    const id = String(parseInt(get().requests[0]?.id || "0") + 1).padStart(
      3,
      "0",
    );
  ...
  updateApartmentContact: (id, data) =>
    set((s) => ({
